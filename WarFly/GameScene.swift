...
class GameScene: SKScene {
    ...
    fileprivate func spawnEnemy(count: Int) {
        ...
        SKTextureAtlas.preloadTextureAtlases([enemyTextureAtlas]) {
            ...
            let spwanAction = SKAction.sequence([waitAction, spawnEnemy])
            let repeatAction = SKAction.repeat(spwanAction, count: count)
    ...
    fileprivate func spawnClouds() {
        ...
        let spawnClowdAction = SKAction.run {
            let cloud = Cloud.populate(at: nil)
            self.addChild(cloud)
        }
        let spawnCloudSequence = SKAction.sequence([spawnCloudWait, spawnClowdAction])
